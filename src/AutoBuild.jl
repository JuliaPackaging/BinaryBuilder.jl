...
function build_jll_package(src_name::String,
    ...
    print_source(io, s::FileSource) =    println(io, "* remote file: ", s.url, " (SHA256 checksum: `", s.hash,"`)")
    function print_source(io, s::DirectorySource)
        print(io, "* files in local directory: ")
        if is_yggdrasil
            println(io, "[`", s.path, "`](https://github.com/JuliaPackaging/Yggdrasil/tree/", ygg_head, "/", src_name_first, "/", src_name, "/", basename(s.path), ")")
        else
            println(io, "`", s.path, "`")
        end
    end
    ...
    open(joinpath(code_dir, "README.md"), "w") do io
        ...
        if is_yggdrasil
            print(io, " The originating [`build_tarballs.jl`](https://github.com/JuliaPackaging/Yggdrasil/blob/$(ygg_head)/$(src_name_first)/$(src_name)/build_tarballs.jl) script can be found on [`Yggdrasil`](https://github.com/JuliaPackaging/Yggdrasil/), the community build tree.")
        end
        println(io)
        println(io)
