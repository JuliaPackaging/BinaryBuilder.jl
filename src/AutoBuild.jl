    ...
    for platform in sort(collect(platforms), by = triplet)
        ...
        open(joinpath(code_dir, "src", "wrappers", "$(triplet(platform)).jl"), "w") do io
            ...
            if !isempty(products_info)
                println(io, """
                export $(join(variable_name.(first.(collect(products_info))), ", "))
                """)
            end
            ...
            for (p, p_info) in sort(collect(products_info), by = first)
                vp = variable_name(p)
                println(io, """
                # Relative path to `$(vp)`
    ...
    open(joinpath(code_dir, "README.md"), "w") do io
        ...
        if length(keys(products_info)) > 0
            ...
            println(io, """
                        ...
                        """)
            for (p, _) in sort(collect(products_info), by = first)
                print_product(io, p)
            end
        end
    end
