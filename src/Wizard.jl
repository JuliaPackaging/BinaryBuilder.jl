...
function download_source(state, workspace, num)
    ...
    if endswith(url, ".git")
        ...
        println(state.outs, "You have selected a git repository. Please enter a branch, commit or tag to use.")
        println(state.outs, "Please note that for reproducability, the exact commit will be recorded,")
        println(state.outs, "so updates to the remote resource will not be automatically reflected.")
        print(state.outs, "> ")
...
function print_autoconf_hint(state)
...
function setup_workspace(build_path, src_paths, src_hashes, platform, extra_env=Dict{String, String}(); verbose=false, tee_stream=STDOUT)
    ...
    for (src_path, src_hash) in zip(src_paths, src_hashes)
        if isdir(src_path)
            repo_dir = joinpath(build_path,"workspace",nonce,"srcdir",basename(src_path)[1:end-4])
            ...
            cp(src_path, joinpath(build_path,"workspace",nonce,"srcdir",basename(src_path)))
...
function step3_retry(state)
    ...
    local ok = true
    cd(build_path) do
        prefix, ur = setup_workspace(build_path, state.source_files, state.source_hashes,
            platform; verbose=true, tee_stream=state.outs)
...
function step34(state)
    ...
    cd(build_path) do
        histfile = joinpath(build_path, ".bash_history")
        prefix, ur = setup_workspace(build_path, state.source_files,
            state.source_hashes, Linux(:x86_64),
            Dict("HISTFILE"=>"/workspace/.bash_history");
            verbose=true, tee_stream=state.outs)
...
function step5_internal(state, platform, message)
    ...
    cd(build_path) do
        prefix, ur = setup_workspace(build_path, state.source_files,
            state.source_hashes, platform; verbose=true, tee_stream=state.outs)
...
function step5c(state)
    ...
    for platform in filter(x->!(x in (Linux(:x86_64), Linux(:aarch64), Windows(:x86_64))),
        ...
        cd(build_path) do
            prefix, ur = setup_workspace(build_path, state.source_files,
                state.source_hashes, platform; verbose=false, tee_stream=state.outs)
