...
@testset "Prefix" begin
    rm("./testing_prefix"; force=true, recursive=true)
    prefix = BinDeps2.Prefix("./testing_prefix")

    # Test that it's taking the absolute path
    @test prefix.path == abspath("./testing_prefix")
    ...
    # Clean up after our disgusting selves
    rm("./testing_prefix"; force=true, recursive=true)
end
...
@testset "Dependency" begin
    ...
    # Clear out the prefix as we're rebuilding the same stuff as above
    rm("./testing_prefix"; force=true, recursive=true)
    prefix = BinDeps2.Prefix("./testing_prefix")
...
@testset "Packaging" begin
    rm("./libfoo_build_prefix"; force=true, recursive=true)
    rm("./libfoo.tar.gz"; force=true)
    build_prefix = BinDeps2.Prefix("./libfoo_build_prefix")
