...
@testset "GitHub releases build.jl reconstruction" begin
    ...
    @test length(product_hashes) == length(true_ogg_hashes)
    for target in keys(true_ogg_hashes)
        @test haskey(product_hashes, target)
        product_platform = extract_platform_key(product_hashes[target][1])
        true_ogg_platform = extract_platform_key(true_ogg_hashes[target][1])
        @test product_platform == true_ogg_platform
        @test product_hashes[target][2] == true_ogg_hashes[target][2]
    end
end
