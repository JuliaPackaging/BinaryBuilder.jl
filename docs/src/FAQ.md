...
### How do I link against BLAS/LAPACK?

Generally, it is recommended to link against [`libblastrampoline`](https://github.com/JuliaLinearAlgebra/libblastrampoline) for BLAS-using libraries. `libblastrampoline_jll` exports both the 32-bit (LP64) and 64-bit (ILP64) interfaces for BLAS/LAPACK, allowing applications which use one or the other, or both, to link against the library. The `CMakeLists.txt` or `Makefile` for building your library should be modified to instead link against `blastrampoline`. On Windows, the name of the built library is `blastrampoline-5`, so this slightly-different name will need to be accounted for either in the `CMakeLists.txt`/`Makefile`, or in the build script where the name could also just be changed to `libblastrampoline.dll`.

Alternatively, for the ILP64 interface the binary dependency `LAPACK_jll` can be used with either `ReferenceBLAS_jll` ([Netlib Reference BLAS](https://www.netlib.org/blas/)) or `OpenBLAS_jll` ([OpenBLAS](https://www.openblas.net/)), or for the LP64 interface `LAPACK32_jll` and either `ReferenceBLAS32_jll` or `OpenBLAS32_jll`. With this option, `cmake` will find `LAPACK` with just `find_package(LAPACK)`.
